...
std::string executablePath() {
    ...
    ::WideCharToMultiByte(CP_UTF8, 0, buf, -1, &result[0],
                          static_cast<int>(result.size() + 1U), nullptr, nullptr);
    return result;
#else
    return {};
#endif
...
std::vector<std::string> findStdlibRoots() {
    ...
    if (env != nullptr) {
        std::string path(env);
#if defined(ZITH_IS_WASM) && defined(_WIN32)
        std::free(env);
#endif
#ifdef ZITH_IS_WASM
