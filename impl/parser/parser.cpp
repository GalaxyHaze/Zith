...
static KalidousNode *parse_union_decl(Parser *p, KalidousVisibility vis, bool is_raw);
...
KalidousNode *parser_parse_statement(Parser *p) {
    ...
    switch (parser_peek(p)->type) {
        case KALIDOUS_TOKEN_LET: parser_advance(p);
            return parse_var_decl(p, KALIDOUS_BINDING_LET);
        case KALIDOUS_TOKEN_VAR: parser_advance(p);
            return parse_var_decl(p, KALIDOUS_BINDING_VAR);
        case KALIDOUS_TOKEN_AUTO: parser_advance(p);
            return parse_var_decl(p, KALIDOUS_BINDING_AUTO);
        case KALIDOUS_TOKEN_CONST: parser_advance(p);
...
static KalidousNode *parse_param(Parser *p) {
    ...
    else if (parser_match(p, KALIDOUS_TOKEN_LEND)) {
        own = KALIDOUS_OWN_LEND;
        is_mutable = true;
    }
    const KalidousToken *name = parser_expect(p, KALIDOUS_TOKEN_IDENTIFIER,
